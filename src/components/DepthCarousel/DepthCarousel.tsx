...
// Import nomeado (não default) de propósito: é a forma que faz o TypeScript
// mesclar corretamente o namespace de tipos do gsap (usado abaixo como
// `gsap.core.Tween`) — com `import gsap from 'gsap'` (como o .jsx original)
// esse namespace de tipos não fica disponível de forma confiável.
...
function DepthCarousel({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current
      const needed = cfg.cardWidth + Math.abs(spread) * 2 + 120
  ...
  return (
    ...
    >
      ...
      {showControls && count > 1 && (
        <>
          <button type="button" className="depth-carousel__arrow depth-carousel__arrow--prev" aria-label="Slide anterior" onClick={() => navigateBy(-1)}>
            <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
              <path d="M15 5l-7 7 7 7" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <button type="button" className="depth-carousel__arrow depth-carousel__arrow--next" aria-label="Próximo slide" onClick={() => navigateBy(1)}>
            <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
